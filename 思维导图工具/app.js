window.onload = function() {
    ...
    var viewerUrl = 'http://127.0.0.1:8080/pdfjs-3.7.107-dist/web/viewer.html?file=' + defaultPdfUrl;
    ...
    document.getElementById('select-pdf').addEventListener('change', function(e) {
        ...
        .then(function(response) {
            ...
            var viewerUrl = 'http://localhost:8080/pdfjs-3.7.107-dist/web/viewer.html?file=' + pdfUrl;
...
function flattenData(data, parentId = null, level = 0) {
    ...
    document.getElementById('draw').addEventListener('click', function() {
        ...
        try {
            ...
            container.innerHTML = "<p style='color:red;'>JSON parsing error: " + e.message + "</p>" + 
            "<pre>" + inputText.replace(/</g, "&lt;").replace(/>/g, "&gt;") + "</pre>";
            return;
        }
        ...
        // 更新下载链接
        var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(inputText);
    ...
    document.getElementById('import-mindmap').addEventListener('click', function() {
    ...
    function updateTokenCount() {
        ...
        if (tokenCount === 0) {
            ...
            tokenDisplayElement.textContent = '输入框token数量:0,GPT-4 最大 token：32768';
        } else {
...
var makeGpt4Request = function(prompt, buttonId) {
    ...
    // 发送请求
    return axios.post('http://localhost:8080/ask', {
        text: prompt
    })
    .finally(function() {
        // 清除超时并重新启用按钮
...
document.getElementById('ask').addEventListener('click', function() {
    var inputText = document.getElementById('input-text').value;
    makeGpt4Request(inputText, 'ask').then(function(response) {
        document.getElementById('mindmap-display').textContent = response.data.message;
    }).catch(function(error) {
        console.error('Error:', error);
        var container = document.getElementById('mindmap-display');
...
document.getElementById('generate-pdf').addEventListener('click', function() {
    var inputText = document.getElementById('input-text').value;
    var prompt = '{"原文:{"' + inputText + '假如你是资深的英文教授，请深入逐句阅读本文的内容，并且生成至少四级以上的关键内容思维导图，保证每个关键内容都有一个原文的example例句。输出json格式如下{{"id": "1", "label": "根节点","example":example_text, "children": [{"id": "2", "label": "子节点1","example":example_text, "children": [{"id": "3", "label": "孙节点1","example":example_text, "children": [{"id": "4", "label": "重孙节点1","example":example_text, "children": [{"id": "5", "label": "曾重孙节点1","example":example_text, "children": [{"id": "6", "label": "玄孙节点1","example":example_text}, {"id": "7", "label": "玄孙节点2","example":example_text}]}, {"id": "8", "label": "曾重孙节点2","example":example_text}]}, {"id": "9", "label": "重孙节点2","example":example_text}]}, {"id": "10", "label": "孙节点2","example":example_text}]}, {"id": "11", "label": "子节点2","example":example_text}]}}，一定要进行json语法检查,输出符合Json格式'
    makeGpt4Request(prompt, 'generate-pdf').then(function(response) {
        var container = document.getElementById('mindmap-display');
        var treeData = null;
        try {
            ...
            var network = new vis.Network(container, data, options);
             // 更新下载链接
             var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(treeData, null, 2));
             var downloadLink = document.getElementById('download-link');
             downloadLink.setAttribute("href", dataStr);
